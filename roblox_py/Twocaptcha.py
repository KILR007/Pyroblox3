    ...
    async def solve(self, public_key):
        ...
        async with aiohttp.ClientSession() as f:
            ...
            if data['request'] == "IP_BANNED":
                raise IpBanned("Your IP address is banned due to many frequent attempts to access the server using "
                               "wrong authorization keys. ")
            if data['request'] == "ERROR_IP_NOT_ALLOWED":
                raise IpNotAllowed("The request is sent from the IP that is not on the list of your allowed IPs.")
            ...
            if data['request'] == "ERROR: NNNN":
                raise NNNNError('Where NNNN is numeric error code.'
                                'You exceeded request limit and your account is temporary suspended.')
            if data['request'] == "ERROR_BAD_PROXY":
                raise BadProxy('You can get this error code when sending a captcha via proxy server which is marked '
                               'as BAD by our API')
            ...
            while True:
                ...
                if data_json['requests'] == "ERROR_CAPTCHA_UNSOLVABLE":
                    raise UnsolvableCaptcha("We are unable to solve your captcha - three of our workers were unable "
                ...
                if data_json['requests'] == "REPORT_NOT_RECORDED":
                    raise ReportNotRecorded("Error is returned to your report request if you already complained lots "
                ...
                if data_json['requests'] == "ERROR_IP_ADDRES":
                    raise IpAddressError("You can receive this error code when registering a pingback (callback) IP "
                ...
                if data_json['requests'] == "ERROR_PROXY_CONNECTION_FAILED":
                    raise ProxyConnectionFailed("You can get this error code if we were unable to load a captcha "
